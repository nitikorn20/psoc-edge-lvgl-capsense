...
/* If a widget has `style_opa < 255` (not `bg_opa`, `text_opa` etc) or not
 * NORMAL blend mode it is buffered into a "simple" layer before rendering. The
 * widget can be buffered in smaller chunks. "Transformed layers" (if
 * `transform_angle/zoom` are set) use larger buffers and can't be drawn in
 * chunks. */
 ...
 * NOTE: If FreeType or ThorVG is enabled, it is recommended to set it to 32KB
 * or more.
 */
...
/* 0: use a simple renderer capable of drawing only simple rectangles with
 * gradient, images, texts, and straight lines only 1: use a complex renderer
 * capable of drawing rounded corners, shadow, skew lines, and arcs too */
#define LV_DRAW_SW_COMPLEX 1

#if LV_DRAW_SW_COMPLEX == 1
/*Allow buffering some shadow calculation.
 *LV_DRAW_SW_SHADOW_CACHE_SIZE is the max. shadow size to buffer, where shadow
 * size is `shadow_width + radius` Caching has LV_DRAW_SW_SHADOW_CACHE_SIZE^2
 * RAM cost*/
...
/* Enable drawing complex gradients in software: linear at an angle, radial or
 * conical */
...
/* VG-Lite flush commit trigger threshold. GPU will try to batch these many draw
 * tasks. */
 ...
 *E.g `my_print`. The prototype should be `void my_print(lv_log_level_t level,
 * const char * buf)` Can be overwritten by `lv_log_register_print_cb`*/
 ...
 *Used by image decoders such as `lv_lodepng` to keep the decoded image in the
 * memory. If size is not set to 0, the decoder will fail to decode when the
 * cache is full. If size is 0, the cache function is not enabled and the
 * decoded mem will be released immediately after use.*/
#define LV_CACHE_DEF_SIZE 0

/*Default number of image header cache entries. The cache is used to store the
 * headers of images The main logic is like `LV_CACHE_DEF_SIZE` but for image
 * headers.*/
 ...
 * (blending) differently. 0: round down, 64: round up from x.75, 128: round up
 * from half, 192: round up from x.25, 254: round up */
#define LV_COLOR_MIX_ROUND_OFS 0

/* Add 2 x 32 bit variables to each lv_obj_t to speed up getting style
 * properties */
 ...
 * - lv_obj_assign_id:       Called when a widget is created. Use a separate
 * counter for each widget class as an ID.
 * - lv_obj_id_compare:      Compare the ID to decide if it matches with a
 * requested value.
 * - lv_obj_stringify_id:    Return e.g. "button3"
 * - lv_obj_free_id:         Does nothing, as there is no memory allocation  for
 * the ID. When disabled these functions needs to be implemented by the user.*/
...
/*Include `lvgl_private.h` in `lvgl.h` to access internal data and functions by
 * default*/
 ...
 *In these languages characters should be replaced with another form based on
...
/*Setting a default driver letter allows skipping the driver prefix in
 * filepaths*/
...
#define LV_FS_ARDUINO_ESP_LITTLEFS_LETTER                                      \
  '\0' /*Set an upper cased letter on which the drive will accessible (e.g.    \
          'A')*/
#endif
...
#define LV_FS_ARDUINO_SD_LETTER                                                \
  '\0' /*Set an upper cased letter on which the drive will accessible (e.g.    \
          'A')*/
#endif
...
/*Cache count of the glyphs in FreeType. It means the number of glyphs that can
 * be cached. The higher the value, the more memory will be used.*/
...
/*0: Displays performance data on the screen, 1: Prints performance data using
 * log.*/
...
/*If you do not use the default thesaurus, be sure to use `lv_ime_pinyin` after
 * setting the thesaurus*/
...
#define LV_SDL_RENDER_MODE                                                     \
  LV_DISPLAY_RENDER_MODE_DIRECT /*LV_DISPLAY_RENDER_MODE_DIRECT is recommended \
                                   for best performance*/
#define LV_SDL_BUF_COUNT 1      /*1 or 2*/
#define LV_SDL_ACCELERATED 1    /*1: Use hardware acceleration*/
#define LV_SDL_FULLSCREEN 0     /*1: Make the window full screen by default*/
#define LV_SDL_DIRECT_EXIT                                                     \
  1 /*1: Exit the application when all SDL windows are closed*/
#define LV_SDL_MOUSEWHEEL_MODE                                                 \
  LV_SDL_MOUSEWHEEL_MODE_ENCODER /*LV_SDL_MOUSEWHEEL_MODE_ENCODER/CROWN*/
#endif

/*Use X11 to open window on Linux desktop and handle mouse and keyboard*/
#define LV_USE_X11 0
#if LV_USE_X11
#define LV_X11_DIRECT_EXIT                                                     \
  1 /*Exit the application when all X11 windows have been closed*/
#define LV_X11_DOUBLE_BUFFER 1 /*Use double buffers for rendering*/
/*select only 1 of the following render modes (LV_X11_RENDER_MODE_PARTIAL
 * preferred!)*/
...
#if LV_USE_WAYLAND
#define LV_WAYLAND_WINDOW_DECORATIONS                                          \
  0 /*Draw client side window decorations only necessary on Mutter/GNOME*/
#define LV_WAYLAND_WL_SHELL                                                    \
  0 /*Use the legacy wl_shell protocol instead of the default XDG shell*/
#endif
...
#define LV_USE_GENERIC_MIPI                                                    \
  (LV_USE_ST7735 | LV_USE_ST7789 | LV_USE_ST7796 | LV_USE_ILI9341)
