...
static lv_obj_t *capsense_label = NULL;
...
static void app_ui_update(const capsense_event_t *event);
...
static void cm55_gfx_task(void *arg)
{
    ...
    TickType_t last_capsense_tick = 0;
    ...
    if (CY_GFX_SUCCESS == gfx_status)
    {
        ...
        if (VG_LITE_SUCCESS == vglite_status)
        {
            ...
            capsense_i2c_init(&disp_touch_i2c_controller_context);
            app_ui_init();
        }
        else
        {
    ...
    for (;;)
    {
        ...
        if ((xTaskGetTickCount() - last_capsense_tick) >=
            pdMS_TO_TICKS(CAPSENSE_POLL_MS))
        {
            last_capsense_tick = xTaskGetTickCount();
            if (capsense_i2c_poll(&capsense_event))
            {
                app_ui_update(&capsense_event);
            }
        }
...
static void app_ui_init(void)
{
    ...
    capsense_label = lv_label_create(scr);
    lv_label_set_text(capsense_label, "CAPSENSE: none");
    lv_obj_align(capsense_label, LV_ALIGN_TOP_LEFT, 10, 10);
}
...
static void app_ui_update(const capsense_event_t *event)
{
    ...
    if ((capsense_label == NULL) || (event == NULL))
    {
        return;
    }
    ...
    switch (event->type)
    {
        case CAPSENSE_EVENT_BTN0:
            (void)snprintf(text, sizeof(text), "CAPSENSE: BTN0");
            break;
        case CAPSENSE_EVENT_BTN1:
            (void)snprintf(text, sizeof(text), "CAPSENSE: BTN1");
            break;
        case CAPSENSE_EVENT_SLIDER:
            (void)snprintf(text, sizeof(text), "CAPSENSE: SLIDER=%u",
                           (unsigned int)event->slider);
            break;
        default:
            (void)snprintf(text, sizeof(text), "CAPSENSE: none");
            break;
    }

    lv_label_set_text(capsense_label, text);
}
