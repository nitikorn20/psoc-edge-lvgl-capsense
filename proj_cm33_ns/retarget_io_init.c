...
static mtb_syspm_uart_deepsleep_context_t retarget_io_syspm_ds_context = {
    ...
    .rts_pin = {.port = DEBUG_UART_RTS_PORT,
...
static cy_stc_syspm_callback_params_t retarget_io_syspm_cb_params = {
    .context = &retarget_io_syspm_ds_context, .base = CYBSP_DEBUG_UART_HW};
...
void init_retarget_io(void) {
  ...
  result = (cy_rslt_t)Cy_SCB_UART_Init(
      CYBSP_DEBUG_UART_HW, &CYBSP_DEBUG_UART_config, &DEBUG_UART_context);
